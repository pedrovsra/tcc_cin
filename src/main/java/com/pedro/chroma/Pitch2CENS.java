...
package com.pedro.chroma;

import java.util.ArrayList;

import com.pedro.chroma.Consts.ChromaConsts;
		...
		if (applyLogNormalize) {
			for (int i = 0; i < f_pitch_energy.size(); i++) {
				for (int j = 0; j < seg_num; j++)
					f_pitch_energy.get(i)[j] = Math.log10(
							ChromaConsts.addTermLogCompr + f_pitch_energy.get(i)[j] * ChromaConsts.factorLogCompr);
			}
		}
