...
public class MeuViterbi {
	...
	/**
	 * @param PCPs
	 *            sequencia de observacoes Y = (y1, y2, ..., yT) tal que yT == i
	 *            se a observacao no tempo t � oi
	 * @param estados
	 *            S = (s1, s2, ..., sK)
	 * @param matriz_transicao
	 *            A de tamanho K*K tal que A[i,j] � a probabilidade de
	 *            transicao do estado si para o estado sj
	 * @param matriz_emissao
	 *            B de tamanho K*N tal que B[i,j] � a probabilidade de
	 *            observar oj a partir do estado si
	 * @param probs_iniciais
	 *            PI de tamanho K tal que PIi � a probabilidade de x1 = si
	 */
	public String[] run(PitchClassProfile[] PCPs, String[] estados, double[][] matriz_transicao,
			double[][] matriz_emissao, double[] probs_iniciais) {
		...
		for (int i = 0; i < this.K; i++) { // para cada estado Si
			this.T1[i][0] = probs_iniciais[i] * matriz_emissao[i][0]; // T1[i,1]
																		// <-
																		// PI[i]
																		// *
																		// B[i,
		...
		for (int i = 1; i < this.T; i++) { // para i = 2 ate T
			...
			for (int j = 0; j < this.K; j++) { // para cada estado sj
				this.T1[j][i] = maxk(i, j); // T1[i,j] <- maxk (T1[k, i-1] *
											// A[k,j] * B[j, yi]
				this.T2[j][i] = argmaxk(i, j); // T2[i,j] <- argmaxk(T1[k, i-1]
												// * A[k,j])
			}
		}
